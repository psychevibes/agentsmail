...
// ── Anti-abuse: blocked email domains (disposable/throwaway) ──
const BLOCKED_EMAIL_DOMAINS = new Set([
  // Known abuse sources
  'sharebot.net',
  // Disposable email services
  'lxbeta.com', 'duoley.com', 'pazard.com', 'soco7.com', 'onbap.com', 'qvmao.com',
  'mail2art.com', 'bdcimail.com', 'usaaxa.com',
  'tempmail.com', 'throwaway.email', 'guerrillamail.com', 'guerrillamailblock.com',
  'mailinator.com', 'yopmail.com', 'trashmail.com', 'trashmail.me',
  'sharklasers.com', 'grr.la', 'dispostable.com',
  '10minutemail.com', 'temp-mail.org', 'fakeinbox.com', 'tempail.com',
  'mohmal.com', 'guerrillamail.info', 'guerrillamail.net', 'guerrillamail.de',
  'emailondeck.com', 'getnada.com', 'burnermail.io', 'mailnesia.com',
  'maildrop.cc', 'harakirimail.com', 'inboxbear.com',
  'mailcatch.com', 'meltmail.com', 'mintemail.com',
  'tempmailaddress.com', 'throwam.com', 'tmail.ws',
  'tmpmail.net', 'tmpmail.org', 'wegwerfmail.de',
  'mytemp.email', 'cs.email', 'crazymailing.com',
])

// ── Anti-abuse: blocked mailbox name patterns (phishing/scam) ──
const BLOCKED_MAILBOX_PATTERNS = [
  /votre[-_.]?service[-_.]?client/i,
  /service[-_.]?client/i,
  /support[-_.]?technique/i,
  /securite[-_.]?compte/i,
  /verification[-_.]?compte/i,
  /mise[-_.]?a[-_.]?jour/i,
  ...
  /amazon[-_.]?service/i,
  /netflix[-_.]?support/i,
  /microsoft[-_.]?verify/i,
]

function isBlockedMailboxName(name) {
  return BLOCKED_MAILBOX_PATTERNS.some(re => re.test(name))
}
...
async function authenticate(request, env) {
  ...
  try {
    ...
    if (account.status === 'suspended' || account.status === 'banned') return null
    return account
  } catch {
    return null
  }
...
function checkAccountActive(account, env) {
  if (account.status === 'suspended') return error('Account suspended. Contact support.', 403, env)
  if (account.status === 'banned') return error('Account banned.', 403, env)
  return null
}
...
async function handleSignup(request, env) {
  ...
  // Block disposable/abuse email domains
  if (isBlockedEmailDomain(email)) {
    await trackEvent('blocked_domain_signup', env)
    return error('This email domain is not allowed. Please use a valid email address.', 403, env)
  }
  ...
  // If first_mailbox requested, validate name but defer creation until verification
  let pendingMailbox = null
  if (first_mailbox) {
    ...
    if (isBlockedMailboxName(localPart)) {
      await trackEvent('blocked_mailbox_name', env)
      return error('This mailbox name is not allowed.', 403, env)
    }
    const taken = await env.MAILBOXES.get(`mailbox:${localPart}@${DOMAIN}`)
    if (taken) return error(`${localPart}@${DOMAIN} is already taken. Try a different name.`, 409, env)
    pendingMailbox = localPart
  }
  ...
  // Store pending mailbox — will be created on email verification
  if (pendingMailbox) {
    account.pending_first_mailbox = pendingMailbox
  }
  ...
  const result = {
    message: 'Account created. Check your email to verify — your mailbox will be created once verified.', account_id: accountId, api_key: apiKey,
    email, plan, limits: account.limits, verified: false,
  }
  if (pendingMailbox) result.pending_mailbox = `${pendingMailbox}@${DOMAIN}`
...
async function handleVerifyAccount(request, env) {
  ...
  // Create pending first mailbox now that account is verified
  let createdMailbox = null
  if (account.pending_first_mailbox) {
    const localPart = account.pending_first_mailbox
    const address = `${localPart}@${DOMAIN}`
    const existing = await env.MAILBOXES.get(`mailbox:${address}`)
    if (!existing) {
      const mailbox = {
        address, account_id: account.id,
        name: localPart, messages: [], created_at: new Date().toISOString(),
      }
      await env.MAILBOXES.put(`mailbox:${address}`, JSON.stringify(mailbox))
      account.mailboxes.push(address)
      createdMailbox = address
    }
    delete account.pending_first_mailbox
  }

  await saveAccount(account, env)
  await env.ACCOUNTS.put(`account:${account.id}`, JSON.stringify(account))

  console.log(`[ACCOUNT] Verified: ${account.email} (${account.id})${createdMailbox ? ` | mailbox created: ${createdMailbox}` : ''}`)
  const result = { message: 'Account verified', verified: true }
  if (createdMailbox) result.mailbox = createdMailbox
...
async function handleCreateMailbox(request, env, ctx) {
  ...
  if (account.verified === false) {
    return error('Account not verified. Verify your email before creating mailboxes.', 403, env)
  }
...
async function handleAdminStats(request, env) {
  ...
  const eventTypes = ['signup', 'email_sent', 'email_received', 'send_failure', 'auth_failure', 'auth_lockout', 'rate_limit_hit', 'oversized_request', 'blocked_domain_signup', 'blocked_mailbox_name', 'account_banned']
...
// POST /api/admin/ban — ban an account and optionally purge its mailboxes
async function handleAdminBan(request, env) {
  ...
  let body
  try { body = await request.json() } catch { return error('Invalid JSON body', 400, env) }
  const { account_id, email: targetEmail, purge_mailboxes } = body || {}

  let account
  if (account_id) {
    const raw = await env.ACCOUNTS.get(`account:${account_id}`)
    if (raw) account = JSON.parse(raw)
  } else if (targetEmail) {
    const ref = await env.ACCOUNTS.get(`email:${targetEmail}`)
    if (ref) {
      const { account_id: aid } = JSON.parse(ref)
      const raw = await env.ACCOUNTS.get(`account:${aid}`)
      if (raw) account = JSON.parse(raw)
    }
  }

  if (!account) return error('Account not found', 404, env)
  ...
  if (purge_mailboxes) {
    for (const addr of (account.mailboxes || [])) {
      await env.MAILBOXES.delete(`mailbox:${addr}`)
      purged++
    }
    account.mailboxes = []
  }
  ...
  await saveAccount(account, env)
  await env.ACCOUNTS.put(`account:${account.id}`, JSON.stringify(account))
  await trackEvent('account_banned', env)

  console.log(`[ADMIN] Banned account: ${account.email} (${account.id}) | purged ${purged} mailboxes`)
  return json({ message: 'Account banned', email: account.email, id: account.id, mailboxes_purged: purged }, 200, env)
}

// POST /api/admin/cleanup — bulk ban accounts by email domain pattern
async function handleAdminCleanup(request, env) {
  ...
  let body
  try { body = await request.json() } catch { return error('Invalid JSON body', 400, env) }
  const { email_domains, mailbox_pattern, dry_run } = body || {}

  if (!email_domains && !mailbox_pattern) {
    return error('Provide email_domains (array) and/or mailbox_pattern (string)', 400, env)
  }

  const domainSet = new Set((email_domains || []).map(d => d.toLowerCase()))
  const mbRegex = mailbox_pattern ? new RegExp(mailbox_pattern, 'i') : null

  // Iterate all accounts
  const results = { matched: 0, banned: 0, mailboxes_purged: 0, accounts: [] }
  let cursor = null
  do {
    const list = await env.ACCOUNTS.list({ prefix: 'account:', limit: 1000, cursor })
    for (const key of list.keys) {
      try {
        ...
        let match = false
        if (domain && domainSet.has(domain)) match = true
        if (!match && mbRegex) {
          match = (acct.mailboxes || []).some(addr => mbRegex.test(addr))
        }

        if (match) {
          results.matched++
          results.accounts.push({ email: acct.email, id: acct.id, mailboxes: acct.mailboxes })

          if (!dry_run) {
            acct.status = 'banned'
            for (const addr of (acct.mailboxes || [])) {
              await env.MAILBOXES.delete(`mailbox:${addr}`)
              results.mailboxes_purged++
            }
            acct.mailboxes = []
            await saveAccount(acct, env)
            await env.ACCOUNTS.put(`account:${acct.id}`, JSON.stringify(acct))
            results.banned++
          }
        }
      } catch { /* skip */ }
  ...
  console.log(`[ADMIN] Cleanup: matched=${results.matched} banned=${results.banned} purged=${results.mailboxes_purged} dry_run=${!!dry_run}`)
...
export default {
  ...
  async email(message, env, ctx) {
    ...
    // Block delivery to banned/suspended accounts
    const ownerRaw = await env.ACCOUNTS.get(`account:${mailbox.account_id}`)
    if (ownerRaw) {
      const owner = JSON.parse(ownerRaw)
      if (owner.status === 'banned' || owner.status === 'suspended') {
        console.log(`Inbound email blocked: account ${mailbox.account_id} is ${owner.status} | ${from} → ${to}`)
        return
      }
    }
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/admin/cleanup' && method === 'POST') return handleAdminCleanup(request, env)
  ...
  async scheduled(event, env, ctx) {
    ...
    // Collect all accounts with details
    const accounts = []
    let cursor = null
    do {
      const list = await env.ACCOUNTS.list({ prefix: 'account:', limit: 1000, cursor })
    ...
    // Collect all mailboxes with message stats
    const mailboxStats = {}  // address → { account_id, total, inbound, outbound, last_activity }
    let mbCursor = null
    do {
      const list = await env.MAILBOXES.list({ prefix: 'mailbox:', limit: 1000, cursor: mbCursor })
      for (const key of list.keys) {
        try {
          const raw = await env.MAILBOXES.get(key.name)
          if (raw) {
            const mb = JSON.parse(raw)
            ...
            mailboxStats[mb.address] = {
              account_id: mb.account_id,
              total: msgs.length,
              inbound,
              outbound,
              last_activity: lastMsg ? (lastMsg.received_at || lastMsg.sent_at || '') : '',
            }
          }
        } catch { /* skip */ }
      }
    ...
    const totalMailboxes = Object.keys(mailboxStats).length

    // Plan breakdown
    const planCounts = {}
    for (const acct of accounts) {
      const p = acct.plan || 'free'
      planCounts[p] = (planCounts[p] || 0) + 1
    }

    // Verified vs unverified
    const verified = accounts.filter(a => a.verified !== false).length
    const unverified = totalAccounts - verified

    // New signups yesterday
    const newSignups = accounts
      .filter(a => a.created_at && a.created_at.startsWith(yesterday))
      .sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''))

    // Build per-account activity summary
    const accountActivity = accounts.map(acct => {
      const mbAddresses = acct.mailboxes || []
      let totalMsgs = 0, totalIn = 0, totalOut = 0, lastAct = ''
      for (const addr of mbAddresses) {
        const s = mailboxStats[addr]
        if (s) {
          totalMsgs += s.total
          totalIn += s.inbound
          totalOut += s.outbound
      ...
      return {
        ...
        mailbox_count: mbAddresses.length,
        mailboxes: mbAddresses,
    ...
    // Top senders (by outbound messages)
    const topSenders = [...accountActivity]
      .sort((a, b) => b.outbound - a.outbound)
      .slice(0, 10)
      .filter(a => a.outbound > 0)

    // Most active (by total messages)
    const mostActive = [...accountActivity]
      .sort((a, b) => b.total_messages - a.total_messages)
      .slice(0, 10)
      .filter(a => a.total_messages > 0)

    // Recently active (by last_activity)
    const recentlyActive = [...accountActivity]
      .filter(a => a.last_activity)
      .sort((a, b) => b.last_activity.localeCompare(a.last_activity))
      .slice(0, 10)

    // Dormant accounts (have mailboxes but 0 messages)
    const dormant = accountActivity.filter(a => a.mailbox_count > 0 && a.total_messages === 0)
    ...
    const lines = [
      ...
      ``,
      `PLATFORM TOTALS`,
      `  Accounts:   ${totalAccounts}  (verified: ${verified}, unverified: ${unverified})`,
      ...
      ``,
      `ABUSE PREVENTION`,
      `  Blocked domain signups:  ${stats.blocked_domain_signup}`,
      `  Blocked mailbox names:   ${stats.blocked_mailbox_name}`,
      `  Accounts banned:         ${stats.account_banned}`,
      `  Banned accounts total:   ${accounts.filter(a => a.status === 'banned').length}`,
      `  Suspended accounts:      ${accounts.filter(a => a.status === 'suspended').length}`,
    ]

    // New signups section
    if (newSignups.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `NEW SIGNUPS YESTERDAY (${newSignups.length})`)
      for (const s of newSignups) {
        const mbList = (s.mailboxes || []).join(', ') || '(no mailbox)'
        lines.push(`  ${s.email}${s.name ? ` (${s.name})` : ''} — ${mbList}`)
      }
    }

    // Top senders
    if (topSenders.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `TOP SENDERS (by outbound emails stored)`)
      for (const a of topSenders) {
        lines.push(`  ${a.email.padEnd(35)} sent: ${String(a.outbound).padStart(4)}  recv: ${String(a.inbound).padStart(4)}  mailboxes: ${a.mailbox_count}`)
      }
    }

    // Most active
    if (mostActive.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `MOST ACTIVE USERS (by total messages)`)
      for (const a of mostActive) {
        lines.push(`  ${a.email.padEnd(35)} total: ${String(a.total_messages).padStart(4)}  (in: ${a.inbound}, out: ${a.outbound})  plan: ${a.plan}`)
      }
    }

    // Recently active
    if (recentlyActive.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `RECENTLY ACTIVE`)
      for (const a of recentlyActive) {
        const ago = a.last_activity.slice(0, 16).replace('T', ' ')
        lines.push(`  ${a.email.padEnd(35)} last: ${ago}  mailboxes: ${a.mailboxes.join(', ')}`)
      }
    }

    // Unverified accounts
    const unverifiedAccts = accountActivity.filter(a => !a.verified)
    if (unverifiedAccts.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `UNVERIFIED ACCOUNTS (${unverifiedAccts.length})`)
      for (const a of unverifiedAccts.slice(0, 15)) {
        lines.push(`  ${a.email.padEnd(35)} signed up: ${(a.created_at || '').slice(0, 10)}  mailboxes: ${a.mailbox_count}`)
      }
      if (unverifiedAccts.length > 15) lines.push(`  ... and ${unverifiedAccts.length - 15} more`)
    }

    // Dormant accounts
    if (dormant.length > 0) {
      lines.push(``, `${'─'.repeat(50)}`, `DORMANT ACCOUNTS (have mailboxes, 0 messages): ${dormant.length}`)
      for (const a of dormant.slice(0, 10)) {
        lines.push(`  ${a.email.padEnd(35)} mailboxes: ${a.mailboxes.join(', ')}  signed up: ${(a.created_at || '').slice(0, 10)}`)
      }
      if (dormant.length > 10) lines.push(`  ... and ${dormant.length - 10} more`)
    }

    // Full user list
    lines.push(``, `${'─'.repeat(50)}`, `ALL ACCOUNTS (${totalAccounts})`)
    lines.push(`  ${'Email'.padEnd(35)} ${'Plan'.padEnd(10)} ${'V'.padEnd(3)} ${'MB'.padEnd(4)} ${'In'.padEnd(5)} ${'Out'.padEnd(5)} ${'Total'.padEnd(6)} Signed Up`)
    lines.push(`  ${'─'.repeat(85)}`)
    const sorted = [...accountActivity].sort((a, b) => b.total_messages - a.total_messages)
    for (const a of sorted) {
      lines.push(`  ${a.email.padEnd(35)} ${a.plan.padEnd(10)} ${(a.verified ? 'Y' : 'N').padEnd(3)} ${String(a.mailbox_count).padStart(2).padEnd(4)} ${String(a.inbound).padStart(4).padEnd(5)} ${String(a.outbound).padStart(4).padEnd(5)} ${String(a.total_messages).padStart(5).padEnd(6)} ${(a.created_at || '').slice(0, 10)}`)
    }

    lines.push(``, `${'═'.repeat(50)}`, `— AgentsMail Admin`)
    ...
    const subject = hasSecurityEvents
      ...
      : `AgentsMail Report ${yesterday} — ${totalAccounts} users, ${stats.email_sent} sent, ${stats.email_received} received`
